...
def main():
    ...
    myParameters = BOLDparameters.ParametersDiscrete3D(
        ...
        #GRID PARAMETERS
        N=100,              #number of grid points on which the spins will sample offset and vessel mask
        padding=50,          #zero padding added during FFT convolution (not used otherwise) 
    ...
    mySimulation = BOLDsimulation.SimulationDiscrete3D(myParameters, IV=True, voxelseed=None, spinseed=None) #when IV set to True, will include intravascular effects
    ...
    signal, time_range, sim_time = mySimulation.signal_simulation(progressbar=True, offset_method='FFTFromVoxel', extend=True) #progressbar toggles the terminal progress monitoring
